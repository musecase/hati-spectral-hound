...
no autonomous action. An authorized event can trigger a weak, dark Tuya diffuser
burst for no more than five seconds, followed by unconditional shutdown attempts
and an off-state check.
...
manual `/deploy` command. Feedback can propose a better classifier policy, but a
candidate is promoted only after it corrects a known miss with zero regressions
in a controlled evaluation.

## How we built it

- Python 3.12 with typed records, JSON traces, OpenCV motion detection, and 44
  automated tests.
- Authenticated Foscam JPEG capture with RTSP fallback and authenticated `/24`
  rediscovery when DHCP changes the address.
...
- Local Tuya control with weak mode, forced-dark operation, a five-second hard
  cap, shutdown in failure paths, and verified off state.
...
camera Wi-Fi problem, discovered the changing local IP, chose snapshot-first
capture after RTSP stalled, mapped the diffuser from water-only observations, and
turned each red-team question into a regression test.
...
was temperamental, and DHCP changed its address. The diffuser had a consumer app
but no project-specific API. Live wildlife also refuses to respect hackathon demo
schedules. We solved these with authenticated discovery, snapshot-first capture,
Tuya device mapping, synthetic judge fixtures, an honest replay plan, and a plush
false-positive test—without luring an animal.
...
- A real five-frame indoor event was captured and classified in one GPT-5.6 call.
  ...
  `DENY / HUMAN_VETO`.
- The physical diffuser completed bounded water-only weak/dark tests and was
...
Move the camera and Wi-Fi extender to the coop, finish the Telegram bot's live
registration, collect owner-reviewed night events, calibrate the protected zone,
and expand the evaluation set before enabling unattended field operation. Later
versions can vary the deterrent pattern to reduce habituation while preserving the
...
“The camera, GPT-5.6 event, safety decision, and water-only actuator are real;
the wildlife scenarios are explicitly controlled because I declined to bait a
raccoon for a hackathon.”
