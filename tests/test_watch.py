...
class WatchTests(unittest.TestCase):
    def test_rearm_waits_for_consecutive_quiet_samples_before_event(self) -> None:
        baseline = np.zeros((100, 100, 3), dtype=np.uint8)
        moving_a = baseline.copy()
        moving_a[10:50, 10:50] = 255
        moving_b = baseline.copy()
        moving_b[50:90, 50:90] = 255
        trigger = baseline.copy()
        trigger[20:80, 20:80] = 255
        frames = [
            baseline,
            moving_a,
            moving_b,
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            motion = MotionConfig(
                ...
                rearm_quiet_samples=2,
            )
            event, _trace_path, result = watch_once(
                CameraConfig(camera_id="test-camera", host="192.0.2.10"),
                events,
                motion,
                "viewer",
                "secret",
        ...
        self.assertTrue(
            any("local comparisons only, no model calls" in item for item in statuses)
        )
        self.assertTrue(any("Quiet check 0/2" in item for item in statuses))
        self.assertTrue(any("Quiet check 2/2" in item for item in statuses))
